...
static int getInt(int * pNnumeroEntero){
	...
	if(myGets(auxCadena, sizeof(auxCadena)) == 0 && esNumstr(auxCadena))//Error hacer un isnum
	{
		retorno = 0;
...
static int getFloat(float * pNumeroFlotante){
	...
	if(myGets(auxCadena, sizeof(auxCadena)) == 0 && esNumstr(auxCadena) )
	{
		retorno = 0;
...
int getAlnumstr(char* cadenaAlnum){
	int retorno = -1;
	char auxCadena[51];
